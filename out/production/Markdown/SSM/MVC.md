...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/7a8a27c2-ca07-4d6a-98cf-f95f3d57217a.png)

### 过滤器和拦截器的区别？

在 **Web 开发** 中，**过滤器**（Filter）和 **拦截器**（Interceptor）都是用于处理请求和响应的机制，但它们有一些重要的区别。它们的主要作用是处理请求的预处理或后处理，可以用来进行日志记录、权限验证、参数校验、跨域请求处理等操作。尽管有相似之处，但它们在应用场景、工作原理和使用方式上存在一些不同。

**1. 定义**

- **过滤器（Filter）**：
  - 过滤器是基于 Servlet 规范的一部分，它在 **Servlet 容器** 中对请求和响应进行处理。过滤器通常用于处理请求和响应的前后阶段，比如对请求进行编码转换、日志记录、权限校验等。
  - 过滤器是在服务器端实现的，处理的是 HTTP 请求和响应的过滤，不与应用程序业务逻辑紧密耦合。
- **拦截器（Interceptor）**：
  - 拦截器一般用于 **框架层**（如 SpringMVC）中，是针对方法调用的前后处理机制。拦截器可以在处理请求前和请求后执行某些操作，主要用于方法级别的控制和增强，比如事务管理、日志记录、权限校验等。
  - 拦截器是框架层的扩展机制，通常和应用程序的业务逻辑紧密结合。

**2. 工作原理**

- **过滤器**：
  - 过滤器在请求到达目标资源（如 Servlet、JSP 页面等）之前进行处理，或者在响应返回客户端之前进行处理。过滤器通过链式的方式顺序处理，可以根据需要对请求和响应进行修改。
  - 过滤器可以通过 `doFilter` 方法链式传递请求和响应。
  - 它是基于 **Servlet 容器** 实现的。
- **拦截器**：
  - 拦截器通常通过代理模式实现，可以拦截方法的调用。它在方法执行前和执行后进行处理。
  - 在 **SpringMVC** 中，拦截器通常使用 `HandlerInterceptor` 接口定义，拦截器通过配置拦截规则来控制哪些请求需要被拦截。
  - 它是基于 **框架（如 Spring）** 实现的。
  - 

**3. 触发时机**

- **过滤器**：
  - 在请求到达 `Servlet` 前（前置过滤）或者在响应返回客户端之前（后置过滤）执行。
  - 过滤器在 Servlet 容器层面生效，因此它不依赖于框架，如 Spring 或 Struts 等。
- **拦截器**：
  - 通常在请求到达 **Controller** 方法之前执行（前置），或者在方法执行完成后执行（后置）。在 Spring 中，拦截器可以影响请求和响应的整个生命周期，但通常更关注于控制器层的请求处理。

**4. 配置方式**

- 过滤器
  - 过滤器的配置通常是通过 **`web.xml`** 文件（在传统的 Servlet 容器中）或 **注解** 来配置的。在 Spring Boot 中，通常使用 `FilterRegistrationBean` 注册过滤器。
- 拦截器
  - 拦截器在 Spring 等框架中通过配置类进行注册。比如，在 Spring 中需要实现 `HandlerInterceptor` 接口并通过 `WebMvcConfigurer` 配置类进行注册。
