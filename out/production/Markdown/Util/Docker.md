...
Docker 是一种用于开发、交付和运行应用程序的开源平台，它通过容器化技术来实现应用程序的隔离和轻量化部署。Docker 的核心原理依赖于 Linux 的一些关键技术，如 **命名空间 (namespace)**、**控制组 (cgroups)** 和 **联合文件系统 (UnionFS)**。

**Docker 的工作原理**

​	1.**容器与虚拟机的区别**：

​	•**虚拟机 (VM)**：虚拟机通过模拟完整的硬件运行一个操作系统，每个虚拟机有自己的内核和完整操作系统，资源占用较大。

​	•**容器 (Container)**：Docker 容器与宿主机共享操作系统内核，使用命名空间和控制组对进程和资源进行隔离，因此容器比虚拟机更加轻量，启动速度快，资源开销小。

​	2.**关键技术**：

Docker 的核心技术依赖于 Linux 内核的三大机制：**命名空间 (namespace)**、**控制组 (cgroups)** 和 **联合文件系统 (UnionFS)**。

​	•**命名空间 (Namespaces)**：提供进程隔离技术，Docker 通过它来实现容器的进程、网络、文件系统和主机名的隔离。常用的命名空间有：
...
​	•**控制组 (cgroups)**：控制和限制进程可以使用的系统资源，如 CPU、内存、网络 I/O 等，确保容器互不影响。

​	•**联合文件系统 (UnionFS)**：提供高效的文件系统管理，允许容器通过分层的方式共享和复用文件。Docker 镜像和容器是基于 UnionFS 构建的，多个镜像层可以叠加在一起，形成一个最终的容器文件系统。这种机制使得容器镜像具有高效、轻量的特点。

​	3.**Docker 镜像 (Images) 与容器 (Containers)**：

​	•**镜像 (Image)**：Docker 镜像是一个只读模板，包含运行应用程序所需的文件系统和依赖包。镜像是容器的基础，通过镜像可以创建多个容器。

​	•**容器 (Container)**：容器是镜像的运行时实例。每个容器都有自己的文件系统、网络、进程空间，但与其他容器共享操作系统内核。

​	4.**Docker 引擎 (Docker Engine)**：

Docker Engine 是 Docker 的核心组件，负责管理镜像、容器和网络。它由三部分组成：

​	•**服务器 (Daemon)**：dockerd 是 Docker 的后台守护进程，负责创建、运行和管理容器。

​	•**REST API**：允许用户与守护进程进行交互。

​	•**客户端 (CLI)**：用户通过 Docker 命令行与 Docker 引擎进行交互。

​	5.**Docker 网络 (Networking)**：

Docker 提供了多种网络模式来连接容器与外部网络：

​	•**Bridge 网络**：默认网络模式，适用于在单机上隔离容器之间的通信。

​	•**Host 网络**：容器与宿主机共享网络堆栈，性能较好，但没有网络隔离。

​	•**Overlay 网络**：用于跨多台宿主机连接容器，适合分布式应用。
