...
* SQL92 过滤：工作流程和 Tag 过滤大致一样，只是在 Store 层的具体过滤方式不一样。真正的 SQL expression 的构建和执行由 rocketmq-filter 模块负责，每次过滤都去执行 SQL 表达式会影响效率，所以 RocketMQ 使用了 BloomFilter 来避免了每次都去执
...
### 为什么RabbitMQ比RocketMQ慢？

**1. 协议和模型设计的不同**

​	•**RabbitMQ** 基于 **AMQP 协议**，该协议是一个功能丰富、设计复杂的消息协议，支持各种消息模型（如路由、事务、确认机制等）。这些复杂功能虽然带来了灵活性，但也引入了额外的开销。

​	•**AMQP** 协议定义了消息传递中的多个步骤，如消息路由、交换器与队列的绑定关系、消息的持久化和确认。这些操作会消耗更多的系统资源和处理时间。

​	•在高并发和海量消息处理的场景中，复杂的协议和功能支持会导致系统处理消息的能力下降。

​	•**RocketMQ** 则专注于高吞吐量和高性能的消息传递，使用了简化的 **TCP 协议**，并通过减少消息的路由和协议开销，优化了性能。

​	•RocketMQ 的设计目标是高性能，它简化了消息传递的过程，使用更轻量级的队列模型，并且在消息持久化、消息重试等功能上做了极大优化，这使得其处理吞吐量大大高于 RabbitMQ。

**2. 消息存储和持久化机制的不同**

​	•**RabbitMQ** 采用了较为传统的消息持久化机制，消息可以存储在内存或磁盘上，特别是在持久化消息时，RabbitMQ 会将消息同步到磁盘，这增加了 I/O 开销。虽然持久化机制确保了消息可靠性，但也牺牲了部分性能。

​	•RabbitMQ 使用 **Erlang 的 Mnesia 数据库** 作为元数据存储，这虽然有助于提升可扩展性和分布式能力，但其写入和读取性能不如 RocketMQ 这样专门为高并发设计的存储方案。

​	•**RocketMQ** 在持久化方面做了更强的优化，它的存储模型更高效，采用了 **零拷贝**（zero-copy）技术，直接将数据从内存写入磁盘，并且消息文件设计上采取了分片和顺序写的方式，极大减少了随机 I/O 操作，提高了吞吐量。
