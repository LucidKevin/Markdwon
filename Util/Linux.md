...
<img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3d2f4c1ec6a14eca9553784df58d61eb~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp" alt="img" style="zoom:50%;" />
...
`select`、`poll` 和 `epoll` 都是用于处理多个文件描述符（通常是套接字）的 I/O 多路复用机制，它们的作用是让程序能够同时监听多个文件描述符的事件（如可读、可写等），从而避免阻塞在某个单一的文件描述符上。它们的区别主要体现在 **性能**、**可扩展性** 和 **使用方式** 上。

1. **select**

- **工作原理**：

  - `select` 是最早的 I/O 多路复用机制，使用时，用户需要指定一组文件描述符集合，然后传递给内核。内核会检查这些文件描述符，判断哪些文件描述符可以进行读写操作，用户进程根据返回结果决定下一步的操作。
  - `select` 每次调用时，都要将文件描述符集合从用户空间复制到内核空间，检查完后再将结果从内核空间复制回用户空间。

- **优缺点**：

  - 缺点
    - 文件描述符数量有限：`select` 支持的文件描述符数量有限，通常是 1024 个，超过此限制需要修改系统参数。
    - 每次调用时，文件描述符集合需要重新传递给内核，性能开销较大。
    - 不适用于高并发的场景，随着文件描述符数量的增加，效率会显著降低。
  - 优点
    - 简单，广泛支持，所有 POSIX 系统都支持。
...
- **工作原理**：
  ...
  - `poll` 的工作原理类似于 `select`，但它使用一个数组来存储文件描述符和相关事件（如读、写、异常等），而不是使用位图。用户通过 `pollfd` 数组指定要监听的文件描述符以及事件。
  - 内核会检查每个文件描述符的状态，并将结果存储在 `pollfd` 数组中。

- **优缺点**：

  - 缺点
    - 性能与 `select` 相似，随着文件描述符数量的增加，性能也会降低，因为每次调用时需要遍历整个文件描述符数组。
    - 不支持文件描述符数量的动态扩展，尽管没有 `select` 那样的文件描述符数量上限，但效率问题依然存在。
  - 优点
    - 相比 `select`，`poll` 不再有最大文件描述符数量的限制，支持更多的文件描述符。
    - 使用起来比 `select` 更灵活，文件描述符数组可以动态调整。
...
- **工作原理**：
  - `epoll` 是 Linux 特有的高效 I/O 多路复用机制，它克服了 `select` 和 `poll` 的性能瓶颈。
  - `epoll` 采用了 **事件驱动** 的方式，内核只会通知有状态变化的文件描述符，而不是每次都遍历整个文件描述符集合。用户注册感兴趣的文件描述符和事件后，内核只返回发生事件的文件描述符。
  - `epoll` 支持 **边缘触发（Edge Triggered, ET）** 和 **水平触发（Level Triggered, LT）** 两种工作模式，提供了更高的灵活性。
  - `epoll` 不需要每次调用时重新传递文件描述符，文件描述符和事件的注册是在单独的操作中完成的，极大地减少了性能开销。
- **优缺点**：
  - 优点
    - **性能好**：`epoll` 的性能几乎不受文件描述符数量的影响，适合处理大量连接的场景，尤其是高并发的情况下。
    - **支持大规模文件描述符**：没有固定的文件描述符数量限制，可以高效处理成千上万的文件描述符。
    - **事件驱动**：只有发生事件的文件描述符会被通知，避免了遍历所有文件描述符的性能开销。
  - 缺点
    - 仅适用于 Linux，其他操作系统（如 BSD、macOS）不支持 `epoll`。
- **使用方式**：
...
| ------------------ | -------------------------------- | ------------------------ | ---------------------------------- |
| **性能**           | 较差，尤其在文件描述符数量增多时 | 性能略好于 `select`      | 性能最优，适合高并发场景           |
| **文件描述符上限** | 1024（可调整）                   | 没有上限                 | 不受限制                           |
| **系统支持**       | POSIX 标准，广泛支持             | POSIX 标准，广泛支持     | 仅 Linux                           |
| **实现方式**       | 文件描述符集合传递               | 文件描述符数组传递       | 基于事件通知，只有活跃的文件描述符 |
| **适用场景**       | 少量文件描述符的简单应用         | 文件描述符数量适中的应用 | 大量并发连接、高性能应用           |

### **推荐使用场景**：

- **`select`**：适用于小型应用，或者需要兼容多平台的场景。
- **`poll`**：适用于文件描述符数量较多，但性能要求不高的场景。
- **`epoll`**：适用于高并发、大规模文件描述符处理的场景，如 Web 服务器、聊天应用等。
