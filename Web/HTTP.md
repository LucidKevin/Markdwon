...
HTTP（HyperText Transfer Protocol，超文本传输协议）是一种用于在客户端（通常是浏览器）和服务器之间传输超文本的应用层协议。它是万维网的基础，用于传输网页、图像、视频和其他网络资源。HTTP 协议基于请求-响应模型，客户端发送请求，服务器返回响应。
...
1. **客户端发送请求**：
   - 当用户在浏览器中输入网址时，浏览器会向服务器发送 HTTP 请求。请求包含以下部分：
     - **请求行**：包含请求方法（如 `GET`、`POST`）、URL 和 HTTP 版本。
     - **请求头**：包含客户端的信息，如浏览器类型、支持的文件格式、编码方式等。
     - **请求体**：包含客户端发送的任何数据，通常用于 `POST` 请求。
2. **服务器处理请求并返回响应**：
   - 服务器接收到请求后，处理请求并返回一个 HTTP 响应。响应包含以下部分：
     - **状态行**：包含 HTTP 状态码（如 `200 OK` 表示请求成功）和 HTTP 版本。
     - **响应头**：包含服务器信息、内容类型、内容长度等。
     - **响应体**：包含实际传输的数据（如 HTML 文档、图像等）。
3. **客户端接收响应并处理**：
   - 浏览器接收到服务器的响应后，会根据响应内容呈现页面或执行其他操作。
...
* 响应体：页面展示内容, 类似网页的源码
  ...
  <html>
      ...
      <body>
          ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/f330f992-796b-49f9-930e-321d19200719.jpg)
      </body>
  </html>
  ```
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/aa6631df-9873-425f-8fb8-bc4159a3bce7.png)
