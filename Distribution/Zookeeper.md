...
在分布式环境中，经常对应用/服务进行统一命名，便于识别，例如域名相对于 IP 地址更容易被接收![image-20250728173758724](../../../Library/Application Support/typora-user-images/image-20250728173758724.png)
...
- 然后在该目录下创建一个 myid 文件，在文件中添加对应 server 的编号

**2）配置 zoo.cfg 文件**

- 重命名 conf 下 zoo.cfg 文件
- 修改数据存储配置 dataDir=/opt/module/zookeeper-3.5.7/zkData
- 增加如下配置
...
**Paxos** 是由计算机科学家 Leslie Lamport 提出的分布式一致性协议，用于在分布式系统中达成一致性。Paxos 解决了在分布式环境中，即使部分节点出现故障，也能够在多个节点之间达成共识的问题。它是 **共识算法** 的一个典型代表。

#### Paxos 协议的核心概念

- **提议者（Proposer）**：发起提议的节点，负责提出某个值，并试图让其他节点达成共识。
- **接受者（Acceptor）**：投票节点，接受并投票表决提议者提出的值，决定是否接受提议。
- **学习者（Learner）**：学习并最终采纳共识值的节点，主要用于将结果传递给系统的其他部分。

#### Paxos 的过程

Paxos 协议的基本过程分为两个阶段：

1. **准备阶段（Prepare Phase）**：提议者向接受者发送准备请求，询问是否可以接受某个提议编号（proposal number）。
2. **接受阶段（Accept Phase）**：如果提议获得多数接受者的同意，提议者会发送接受请求，要求接受者承诺不再接受更低编号的提议。

一旦达成共识，该值就被确认并被学习者传播到系统的其他部分。

#### Paxos 的优缺点

- **优点**：Paxos 保证了即使在部分节点故障的情况下，仍能够在分布式系统中达成一致性。
- **缺点**：Paxos 协议实现较为复杂，性能也不是特别高，尤其是在需要处理大量节点时，通信开销较大。

***

### ZAB

**ZAB（ZooKeeper Atomic Broadcast）** 是由 Apache ZooKeeper 使用的分布式一致性协议。它的设计目标是实现分布式系统中的 **主从复制和原子广播**，特别适用于需要高可用性和一致性的分布式协调服务。

#### ZAB 的核心特性

- **主从模型**：ZAB 协议基于一个主节点（Leader）和多个从节点（Followers），主节点负责生成提议，从节点则负责复制和确认。
- **崩溃恢复**：ZAB 通过崩溃恢复机制，确保即使在主节点发生故障的情况下，系统依然能够通过选举产生新的主节点，并保持数据一致性。

#### ZAB 的工作流程

ZAB 协议包含两个主要阶段：

1. **原子广播阶段**：主节点生成事务提议，通过原子广播将提议发送给从节点。当大多数从节点确认提议后，提议被提交。
2. **崩溃恢复阶段**：当主节点崩溃时，ZAB 会进入恢复阶段，通过选举新的主节点，并恢复一致性。

ZAB 协议特别适合于领导者选举、分布式锁等需要强一致性的分布式协调任务。

#### ZAB 与 Paxos 的区别

- **应用场景**：ZAB 专为 ZooKeeper 设计，尤其适用于主从复制和需要强一致性的协调任务，而 Paxos 是一个通用的分布式一致性协议。
- **设计目标**：ZAB 更加侧重于在崩溃恢复后快速恢复一致性，并且易于实现主从复制。
...
![image-20250728210146729](./assets/image-20250728210146729.png) 
