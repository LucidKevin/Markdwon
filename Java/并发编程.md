...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/faa60925-e908-4a56-9af1-a4057a0778b6.png)
...
进程的状态参考操作系统：创建态、就绪态、运行态、阻塞态、终止态![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/5445db34-ddf0-4efa-8869-efad41ffb44e.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/768f4608-2efc-4bd7-9be3-2454d039938a.png)  - NEW → RUNNABLE：当调用 t.start() 方法时，由 NEW → RUNNABLE
...
* 32位Mark Word 结构：最后两位是**锁标志位**

  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/ba1f6f94-de43-4aa0-9886-316ede3f0656.png)

* 32位对象头![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/3509842e-adba-488b-98f0-0491a025205d.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/6c12670f-e288-4800-8dca-678c1e20b680.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/8c51228b-5d93-4fc0-a1f3-e933ff313789.png)

* 当 Thread-0 退出同步块解锁时，使用 CAS 将 Mark Word 的值恢复给对象头失败，这时进入重量级解锁流程，即按照 Monitor 地址找到 Monitor 对象，设置 Owner 为 null，唤醒 EntryList 中 BLOCKED 线程

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/08b6ff01-332c-4082-8a84-0be89c288dea.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/636c9a8f-c504-4bd1-b572-036554ce8669.png)
...
* 先 unpark：
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/b3deac94-051b-4381-b244-4632a913ff5d.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/8cd55d56-d99c-4ee6-a742-d258a3f8cfff.png)
...
- 可重入指的是同一个线程如果首次获得了这把锁，那么它是这把锁的拥有者，因此有权利再次获取这把锁，如果是不可重入锁，那么第二次获得锁时，自己也会被锁挡住
...
**CompletableFuture 类深度分析（Java 并发编程）**

`CompletableFuture` 是 Java 8 引入的 **基于事件驱动（callback）模型的异步任务框架**，是 `Future` 的增强版本，提供：
...
它的底层基于 **ForkJoinPool.commonPool() + Completion 回调链表** 来调度执行。

**1. CompletableFuture 的核心设计思想**
...
➡️ **CompletableFuture 解决了 Future 的所有痛点。**
...
**3. CompletableFuture 的执行模型**

**✔️ 3.1 回调链表（Completion）**

每个 `CompletableFuture` 内部维护一个 **Completion 单向链表**：
...
- 触发方式（thenApply、thenAccept…）
- 回调逻辑（fn）
- 下一个节点

当 future 完成时，会从链表头开始 **顺序触发回调**。

➡️ 这也是为啥.thenXXXX() 可以无数个链式调用。
...
如 `thenApplyAsync()` → 回调会提交到线程池异步执行。

| 方法           | 是否依赖上个任务 | 是否有返回值 | 是否异步执行 |
| -------------- | ---------------- | ------------ | ------------ |
| thenApply      | ✔                | ✔            | ❌            |
| thenApplyAsync | ✔                | ✔            | ✔            |
| thenAccept     | ✔                | ❌            | ❌            |
...
**✔️ 5.1 thenCombine（两个任务 AND，如“两个都完成才执行”）**

```
future1.thenCombine(future2, (r1, r2) -> r1 + r2)
```

**✔️ 5.2 applyToEither（两个任务 OR，如“谁先完成用谁”）**

```
future1.applyToEither(future2, x -> x)
```

**✔️ 5.3 allOf（多个任务全部完成）**
...
**7. CompletableFuture 的常见坑点**

**❌ 7.1 默认的 commonPool 容量不足**

commonPool 默认等于 **CPU 核心数 - 1**
 适合 CPU 任务，但不适合大量 IO 任务。
...
**❌ 7.2 thenApply vs thenCompose 混淆**

**thenApply 会得到 `CompletableFuture<CompletableFuture<T>>`**

```
future.thenApply(x -> otherAsync(x))
```

**thenCompose 才是异步扁平化**

```
future.thenCompose(x -> otherAsync(x))
```

完全等价于 JS Promise 的 then / then flatMap。

**❌ 7.3 join() 会抛出 unchecked Exception**

- `get()` 抛 checked Exception
- `join()` 抛 unchecked Exception（CompletionException）
...
- **CompletableFuture**：管理结果、状态
- **Completion**：回调节点
- **ForkJoinPool**：默认线程池
- **UniCompletion / BiCompletion**：one/two parameter 回调节点
...
* 使用了 FIFO 先入先出队列，并不支持优先级队列，**同步队列是双向链表，便于出队入队**
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/736e2ed1-991a-43a4-9a1d-551bac5289fc.png)
...
* 开始 Thread-0 持有锁，调用 await，线程进入 ConditionObject 等待，直到被唤醒或打断，调用 await 方法的线程都是持锁状态的，所以说逻辑里**不存在并发**
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/88d54f39-2251-455b-a88d-f94655848810.png)
...
* fullyRelease 中会 unpark AQS 队列中的下一个节点竞争锁，假设 Thread-1 竞争成功
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/4d5bdad2-9bb3-4845-bb1c-0ecfe7d93472.png)
...
* 执行 transferForSignal，**先将节点的 waitStatus 改为 0，然后加入 AQS 阻塞队列尾部**，将 Thread-3 的 waitStatus 改为 -1
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/a6892631-218b-4aef-835f-642b499e072e.png)
...
* 这种状态下，假设又有 t3 r.lock，t4 w.lock，这期间 t1 仍然持有锁，就变成了下面的样子
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/fe853ce9-d006-4447-921a-5e1c8aad2bdf.png)
...
* Semaphore 的 permits（state）为 3，这时 5 个线程来获取资源
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/3c90e3de-1990-48a0-b204-c626d3da342a.png)
...
* 这时 Thread-4 释放了 permits，状态如下
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/4e6ba041-fb96-44aa-9aef-ec673793be4c.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/e43c0d2d-cbb0-42b5-9db8-cf825bc918ba.png))
...
* transfer()：数据转移到新表中，完成扩容
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/2b664200-4ea4-4c0f-99f0-450630338bde.png)
...
* 缺点：Segments 数组默认大小为16，这个容量初始化指定后就不能改变了，并且不是懒惰初始化
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/fce22917-19d3-4071-8df3-3683a16b0cf2.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/18dc682a-b072-408d-9fe4-4f7f016d10f2.png)
...
* findPredecessor()：寻找前置节点
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/1a08f5d7-1119-4d29-8910-454a0bb0afda.png)
...
* remove()
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/596f1d48-91fe-4569-b2c7-feba3ff2de42.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/e0a295e5-424e-481b-8f23-2d1dfa9c5440.png)

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/d1e7adbf-a84f-4f54-a55c-9c4c586ca58c.png)

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/f5190cbc-a95b-4426-bc31-9fcf347cc2df.png)
...
- 不同的进程使用不同的内存空间，在当前进程下的所有线程可以共享的内存空间
- 线程更清轻量，线程上下文切换成本一般上要比进程上下文切换低
...
- Callable接口的call方法允许抛出异常，而runnable接口的run方法异常只能内部消耗

### 线程有哪些状态，线程状态之间如何变化的？

- 状态：新建(new)，可运行(runnable)，阻塞(blocked)，等待(waiting)，时间等待(time-waiting)，终止(terminated)
- 创建线程对象是**新建状态**
- 调用start()方法转变为**可执行状态**
- 在**可执行状态**过程中，如果没有获取CPU执行权，可能会切换为其他状态
  - 没有获取锁则进入**阻塞状态**
  - 调用wait()则进入等待状态状态
  - 调用sleep(50)则进入计时等待
...
- wait属于Object类，sleep属于线程类
- 调用wait会释放对象锁直到被唤醒，而sleep不会释放锁，通常用于让当前线程暂停执行一段时间

### 如何停止一个正在运行的线程？

- 使用标志变量，可以让线程自行检查状态并决定何时停止，这是一种安全的方式，不会突然中断线程的执行
- 使用interrupt方法，中断不是直接停止线程，而是通知线程应该停止。在适当的地方，线程会抛出异常，可以在捕获异常后进行线程清理
- 使用stop()

### synchronized关键字的底层原理？

- 偏向锁：在第一次获取锁会有CAS操作，将线程ID写入对象头中的mark word，之后再有线程进入，只需查看mark word中线程ID是否相同就可以
- 轻量级锁：每次使用锁都需要创建一个锁记录，然后将锁记录中对象地址与markword进行CAS，如果同一线程则会创建一个空的锁记录
- 重量级锁：底层由monitor（C++实现）实现的，线程获得锁需要使用对象关联monitor，在monitor内部有三个属性，一个是owner，entrylist，waitset
  - 其中owner是关联获得锁的线程
...
Java 内存模型（Java Memory Model, JMM）是Java虚拟机规范的一部分，它定义了多线程环境下如何访问和管理共享内存中的数据。JMM规定了变量（尤其是共享变量）在多线程程序中的可见性和有序性规则，确保了线程间的通信安全性和一致性。

**主要概念**

1. **主内存与工作内存：**
   - **主内存**：所有的实例变量（非局部变量或常量）存储在主内存中，主内存是共享内存区域，所有线程都可以访问。
   - **工作内存**：每个线程都有自己的工作内存，工作内存保存了主内存中变量的副本。线程对变量的操作（读取、写入）必须在工作内存中进行，而不能直接操作主内存。线程之间不能直接访问对方的工作内存，变量值的传递需要通过主内存来完成。
2. **可见性：**
   - 线程对共享变量的修改，什么时候对其他线程可见，这是JMM中可见性规则要解决的问题。比如，某个线程对变量的更新，其他线程能否立刻看到？这是通过 **volatile** 关键字、锁机制、内存屏障等机制来保障的。
   - `volatile` 变量在写入时会立刻刷新到主内存，读取时直接从主内存读取，从而确保可见性。
3. **有序性：**
   - JMM允许编译器和处理器对代码进行重排序优化，但JMM提供了一定的规则来约束这种重排序，保证在多线程环境下不会引入错误的执行顺序。
   - Java中使用 **happens-before** 原则来保证有序性。例如，线程A的操作 "happens-before" 线程B的操作时，线程B可以看到线程A的操作结果。

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/1ef9c386-a18b-4e8a-bd9d-41c48069047e.png)

### 什么是CAS？

CAS是基于乐观锁的的思想的操作，一个当前值V，旧的预期值A，以及即将更新的值B，当且仅当旧的预期值A和内存值V相同时，将内存值修改为B并返回true，否则什么都不做，并返回false。如果CAS失败则通过自旋方式等待并再次尝试

### 什么是volatile？

- 保证线程间的可见性，保证了不同线程对该变量的可见性，即当一个线程修改了volatile变量的值，其他线程能够立即看到这个修改，从而不会重本地缓存中读取过期的值
- 禁止指令重排序 ，`volatile` 还能保证对该变量的操作不会被指令重排优化。对于普通变量，编译器和处理器可能会出于性能优化的考虑进行指令重排，这可能导致多线程环境下的代码执行顺序与预期不一致。而 `volatile` 变量在写入时会插入内存屏障，防止指令重排，以确保变量的读写顺序。

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/d4b46be8-aaea-4e25-bcbb-8e5f0acb2c91.png)

### 什么是AQS？

AbstractQueuedSynchronizer，即抽象队列同步器。它是构建锁或者其他同步组件的基础框架

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/c1900399-80bb-4469-bb0e-d2eccee61ec6.png)

**AQS 的核心概念**

1. **同步状态 (State)**: AQS 内部维护了一个 `int` 类型的同步状态 (`state`)，表示锁的状态或许可的数量。同步器通过对 `state` 的原子操作来控制同步行为。
2. **FIFO 等待队列**: AQS 内部使用了一个双向链表（FIFO 队列）来管理获取锁失败的线程。当线程请求获取同步状态失败时，它会被加入到等待队列中，线程会进入阻塞状态，直到被唤醒。
3. **独占模式 (Exclusive)** 和 **共享模式 (Shared)**: AQS 提供了两种获取同步状态的模式：
   - **独占模式**: 只有一个线程能获取同步状态，其他线程只能等待。例如，`ReentrantLock` 使用的就是独占模式。
   - **共享模式**: 多个线程可以同时获取同步状态。例如，`Semaphore` 使用的就是共享模式。

**AQS 的工作流程**

- **获取同步状态**: 当线程尝试获取锁或进入临界区时，AQS 会通过 `tryAcquire` (独占模式) 或 `tryAcquireShared` (共享模式) 方法尝试获取同步状态。如果获取成功，线程可以继续执行。如果获取失败，线程将被加入等待队列，并进入阻塞状态。
- **释放同步状态**: 当线程完成了工作并准备释放锁或退出临界区时，AQS 会通过 `release` (独占模式) 或 `releaseShared` (共享模式) 方法来释放同步状态。如果有其他线程在等待队列中等待，AQS 会唤醒等待队列中的一个或多个线程。
- **唤醒线程**: 被唤醒的线程会重新尝试获取同步状态。如果成功，线程将继续执行；如果失败，线程将继续等待。 

### ReentrantLock的实现原理？

特点：可中断，可设置超时时间，支持公平锁，支持多个条件变量，支持重入

原理：主要利用CAS+AQS队列来实现，支持公平锁和非公平锁

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/2fb6f0c6-ec69-48be-9527-f352b99f369a.png)
...
使用synchronized时，退出同步代码块锁会自动释放，而使用Lock时，需要手动调整unlock方法释放锁

- 功能层面

二者均属于悲观锁，都具备基本的互斥，同步，锁重入功能

Lock提供了许多synchronized不具备的功能，如公平锁，可打断，可超时

Lock有适合不同场景的实现，如ReetrantLock

- 性能层面

在没有竞争时，synchronized做了很多优化，如偏向锁，轻量级锁，性能不差，在竞争激烈时，Lock的实现通常会提供更好的性能

### 死锁产生的条件？

- 互斥条件：资源在某个时刻只能被一个线程占有
- 请求与保持条件：线程已经拥有一个资源，并发出新的资源请求
- 不剥夺条件：资源不能被剥夺
- 循环等待条件：存在一个线程链，每个进程都在等待下一个进程所持有的资源

### 什么是ConcurrentHashMap？

`ConcurrentHashMap` 是 Java 中的线程安全哈希表实现，用于在多线程环境下高效地执行并发读写操作。它是 Java 中处理并发数据结构的一个重要类，特别适用于在多线程情况下避免使用 `synchronized` 或手动加锁来实现线程安全。以下是 `ConcurrentHashMap` 的详细介绍。

`ConcurrentHashMap` 是基于分段锁（Segment Locking）的机制实现的。与传统的 `Hashtable` 和 `Collections.synchronizedMap()` 不同，`ConcurrentHashMap` 并不会在每次操作时对整个 Map 进行加锁，而是将内部数据划分为多个段（Segements）。这种分段锁机制允许多个线程同时访问不同段的数据，从而提高了并发性能。

- **JDK 7 的实现**：在 JDK 7 中，`ConcurrentHashMap` 采用了 `Segment` 数组，每个 `Segment` 是一个独立的可重入锁（`ReentrantLock`），每个 `Segment` 管理一部分 `HashMap` 的 `Bucket`。这样，多个线程可以同时对不同的 `Segment` 进行并发操作，而不会产生冲突。
- **JDK 8 的实现**：在 JDK 8 中，`ConcurrentHashMap` 不再使用分段锁，而是采用了更加细粒度的锁机制和 `CAS` 操作（Compare-And-Swap）。它使用 `Node` 数组作为基础结构，利用链表或红黑树来处理哈希冲突。同时，通过 `CAS` 和 `synchronized` 锁的组合，确保并发操作的安全性和性能。

### 线程池的核心参数和原理是什么？

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/bdcaf506-7568-4993-a1c0-1c940d58cbf2.png)

- corePoolSize：核心线程数目
- maximumPoolSize：最大线程数目=（核心线程+救急线程的最大数量）

- keepAliveTime：生存时间-救急线程时间单位
- workQueue：当没有空闲核心线程，新来的任务将加入到此队列
- threadFactory：可以制定线程对象的创建
- handler：拒接策略

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/31ecbb32-68a3-478f-b544-edfa437ecd00.png)

### 线程池中有哪些常见的阻塞队列？

1、ArrayBlockingQueue：基于数组结构的有界阻塞队列，FIFO

2、LinkedBlokingQueue：基于链表结构的有界阻塞队列，FIFO

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/95817c1d-0c17-4a7c-ada1-03ec3d22747c.png)

### 如何确定核心线程数？

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/7489de9f-7b62-4a2d-9a0e-b56068d553c1.png)

### 线程池的种类有哪些？

- 创建使用固定线程的线程池，阻塞线程数与最大线程数一样，没有救急线程，用于任务量已知，相对耗时的任务
- 单线程化的线程池，只会用唯一的工作线程来执行任务，核心线程和最大线程数都是1，适用于按照顺序执行的任务
- 可缓存的线程，核心线程数为0，最大线程数为Integer.MAX_Value，阻塞队列为SynchronousQueue，不存元素的阻塞队列，每个插入操作都必须等待一个移出操作，适合任务数比较密集，但每个任务执行时间较短的情况
- 提供延迟和周期执行的线程

### 企业中如何使用线程池？

**并行处理多个独立的任务：**
例如，当有多个相似的任务（如 HTTP 请求、文件处理等）需要并行处理时，使用线程池可以有效提升并发处理能力。

**定时任务调度：**
如果你需要定期执行某些任务，可以使用 `ScheduledExecutorService` 来实现。

### ThreadPoolTaskExecutor 和 ThreadPoolExecutor区别？

ThreadPoolTaskExecutor 和 ThreadPoolExecutor 都是 Java 中用于管理线程池的工具，但二者的设计背景、功能定位和使用场景有显著区别。前者是 Spring 框架对后者的封装与扩展，后者是 JDK 原生的线程池核心实现。以下从核心差异展开说明：

**一、所属框架与设计定位**

- **ThreadPoolExecutor**
  是 JDK 原生类（位于 `java.util.concurrent` 包），是 Java 并发体系中线程池的**核心实现类**，定义了线程池的基本规范（如核心线程数、任务队列、拒绝策略等），所有 Java 线程池的底层逻辑都基于它实现（如 `Executors` 工具类创建的线程池最终都依赖它）。
  定位：提供线程池的基础功能，适用于所有 Java 应用（无论是否使用 Spring）。
- **ThreadPoolTaskExecutor**
  是 Spring 框架（`org.springframework.scheduling.concurrent` 包）提供的类，**本质是对 ThreadPoolExecutor 的封装**，目的是让线程池更贴合 Spring 生态的使用习惯（如 Bean 风格配置、与 Spring 异步任务 `@Async` 集成等）。
  定位：Spring 环境下的线程池工具，简化在 Spring 应用中的线程池配置与使用。

**二、核心差异对比**

| **维度**         | **ThreadPoolExecutor（JDK）**                                | **ThreadPoolTaskExecutor（Spring）**                         |
| ---------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **底层依赖**     | 自身就是线程池的底层实现，直接管理线程生命周期和任务执行。   | 内部通过 `ThreadPoolExecutor` 实例实现核心功能（持有一个 `ThreadPoolExecutor` 成员变量），相当于 “适配器”。 |
| **配置方式**     | 通过**构造函数**配置核心参数（如 `corePoolSize`、`maximumPoolSize`、`workQueue` 等），参数必须在创建时指定。 例： `new ThreadPoolExecutor(2, 5, 60L, TimeUnit.SECONDS, new LinkedBlockingQueue<>())` | 通过**setter 方法**配置参数（如 `setCorePoolSize()`、`setQueueCapacity()`），支持 Spring 容器的依赖注入（如 XML 或 `@Bean` 注解配置），最终通过 `initialize()` 方法创建内部的 `ThreadPoolExecutor`。 例： `taskExecutor.setCorePoolSize(2);` `taskExecutor.setMaxPoolSize(5);` |
| **功能扩展**     | 功能简洁，仅包含线程池核心能力（任务提交、线程管理等）。     | 在 `ThreadPoolExecutor` 基础上增加了 Spring 生态特有的功能： 1. 支持 `TaskDecorator`（任务装饰器，可对提交的任务进行增强，如添加日志、上下文传递）； 2. 支持设置线程名称前缀（`setThreadNamePrefix()`），方便日志跟踪； 3. 集成 Spring 的 `AsyncTaskExecutor` 接口，支持与 `@Async` 注解配合实现异步任务。 |
| **任务提交方式** | 提供 `execute(Runnable)`、`submit(Callable)` 等原生方法。    | 除了支持原生方法，还实现了 Spring 的 `AsyncTaskExecutor` 接口，提供 `submitListenable(...)` 等方法，支持监听任务执行结果（通过 `ListenableFuture`）。 |
| **默认配置**     | 无默认参数，必须手动指定所有核心参数（否则无法创建实例）。   | 提供更合理的默认配置： - 核心线程数默认 1； - 最大线程数默认 `Integer.MAX_VALUE`； - 队列默认 `LinkedBlockingQueue`（容量默认 `Integer.MAX_VALUE`）； - 拒绝策略默认 `CallerRunsPolicy`（任务提交者线程执行任务，避免任务丢失）。 |
| **使用场景**     | 适用于所有 Java 应用，尤其是非 Spring 环境（如纯 Java 项目、Android 等）。 | 仅适用于 Spring 应用，尤其是需要与 Spring 特性（如依赖注入、异步任务、事务管理）集成的场景。 |

**三、本质关系：封装与被封装**

ThreadPoolTaskExecutor 并非重新实现线程池逻辑，而是通过组合模式（持有 `ThreadPoolExecutor` 实例）对其进行封装，核心执行逻辑仍由 `ThreadPoolExecutor` 完成。例如：
当调用 `ThreadPoolTaskExecutor.execute(Runnable)` 时，底层实际调用的是其内部 `ThreadPoolExecutor` 的 `execute` 方法。

可以理解为：`ThreadPoolExecutor` 是 “发动机”，`ThreadPoolTaskExecutor` 是 “汽车”—— 汽车通过整合发动机，并增加方向盘、座椅等部件，使其更适合特定场景（Spring 生态）使用。

**四、如何选择？**

- **如果是 Spring 项目**：优先使用 `ThreadPoolTaskExecutor`，它更符合 Spring 的配置风格（如通过 `@Bean` 配置），且能无缝集成 `@Async` 异步任务、任务装饰器等 Spring 特性。
- **如果是非 Spring 项目**：直接使用 `ThreadPoolExecutor`，它是 JDK 原生工具，无需依赖 Spring 框架，更轻量。
- **如果需要高度定制线程池行为**：两者均可，但 `ThreadPoolExecutor` 允许直接操作核心参数（如自定义 `ThreadFactory`、`RejectedExecutionHandler`），而 `ThreadPoolTaskExecutor` 需通过其提供的 setter 方法间接配置。

**总结**

ThreadPoolExecutor 是 Java 线程池的 “基础实现”，功能原始但通用；ThreadPoolTaskExecutor 是 Spring 对其的 “包装增强”，更适配 Spring 生态。选择时需根据项目是否依赖 Spring 框架，以及是否需要 Spring 特有的扩展功能（如任务装饰、异步监听）来决定。

### SchedulingConfigurer有什么作用？

`SchedulingConfigurer` 是 Spring 框架提供的一个接口，主要用于**自定义定时任务的调度配置**，是 Spring 定时任务机制（基于 `@Scheduled` 注解）的扩展点。它的核心作用是突破 Spring 定时任务的默认配置限制，实现更灵活的调度策略，尤其是线程池配置和动态任务注册。

**核心作用**

1. **自定义定时任务的线程池**
   Spring 中使用 `@Scheduled` 注解的定时任务，默认使用**单线程池**（`SimpleAsyncTaskExecutor`）执行所有任务。这意味着如果一个任务执行时间过长（如阻塞），会导致其他定时任务被延迟或阻塞。

   通过实现 `SchedulingConfigurer`，可以自定义线程池（如设置核心线程数、线程名称、拒绝策略等），让多个定时任务并行执行，避免相互影响。
...
2. **动态注册定时任务**
   `@Scheduled` 注解的定时任务是**静态的**（cron 表达式、执行频率等在代码中写死），无法在运行时修改。

   而 `SchedulingConfigurer` 允许通过 `ScheduledTaskRegistrar` 在**运行时动态注册任务**，例如从数据库读取任务配置（如 cron 表达式），实现定时任务的动态管理（新增、修改、删除）。
   ...
   @Override
   public void configureTasks(ScheduledTaskRegistrar registrar) {
       // 从数据库获取动态配置（实际项目中需查询DB）
       String cron = "0/30 * * * * ?"; // 每30秒执行一次
       
       // 动态注册任务
       registrar.addCronTask(() -> {
           System.out.println("动态注册的定时任务执行了：" + LocalDateTime.now());
       }, cron);
   }
   ```

3. **精细控制任务调度器**
   可以通过 `ScheduledTaskRegistrar` 配置调度器的其他属性，例如任务的执行顺序、异常处理策略等，满足复杂场景的需求。

**使用场景**

- 系统中有**多个定时任务**，需要并行执行（避免单线程阻塞）。
- 定时任务的**执行规则需要动态调整**（如通过后台管理界面修改 cron 表达式）。
- 需要**自定义线程池参数**（如线程名称、拒绝策略、关闭策略等），优化定时任务性能。

**总结**

`SchedulingConfigurer` 是 Spring 定时任务的 “配置入口”，主要解决两个核心问题：

1. 替代默认单线程池，通过自定义线程池实现定时任务的并行执行；
2. 支持动态注册任务，突破 `@Scheduled` 注解的静态限制。

在实际开发中，当系统定时任务较多或需要动态管理时，几乎都会通过实现该接口来优化定时任务的调度机制。
...
![image-20251010192911595](./assets/image-20251010192911595.png)
...
`ThreadLocal` 若使用不当，可能导致内存泄露（线程未销毁，且变量副本未被回收）

1、泄露原因

`ThreadLocalMap` 中 key 是弱引用，value 是强引用，当ThreadLocal 被回收时，key 会变为 null，但是value 不会回收所以导致了内存泄露，因此需要使用完 ThreadLocal 后调用remove 方法
...
悲观锁认为 “数据很可能被并发修改”，因此在**操作数据前会先获取锁**，确保自身独占资源，其他线程只能等待锁释放后才能操作。这是一种 “保守” 的策略，通过独占资源避免冲突。

**实现原理**

1. **加锁时机**：在读取或修改数据前，主动获取锁（如线程级锁、数据库行锁）。
2. **独占性**：锁被一个线程持有后，其他线程尝试获取锁时会被阻塞（进入等待队列）。
3. **释放时机**：操作完成后（正常执行或异常）释放锁，唤醒等待队列中的线程竞争锁。

**典型实现**

- **Java 线程锁**：`synchronized` 关键字、`ReentrantLock` 等，通过 JVM 或 AQS 框架实现线程级别的锁。
- **数据库锁**：`SELECT ... FOR UPDATE`（行级锁）、表锁等，通过数据库机制锁定数据记录。

**二、乐观锁**

**核心思想**

乐观锁认为 “数据并发修改的概率很低”，因此**操作前不上锁**，直接读取并修改数据，仅在**提交更新时检查数据是否被其他线程修改过**。如果未被修改，则成功提交；如果已被修改，则放弃操作（或重试）。这是一种 “乐观” 的策略，通过事后检查解决冲突。

**实现原理**

1. **无锁操作**：读取数据时不加锁，直接记录数据的 “版本标识”（如版本号、时间戳）。
2. 修改与校验：修改数据后，提交更新时检查当前数据的版本标识是否与读取时一致：
   - 一致：说明数据未被其他线程修改，更新成功（同时更新版本标识）。
   - 不一致：说明数据已被修改，更新失败（通常重试或返回错误）。
3. 
