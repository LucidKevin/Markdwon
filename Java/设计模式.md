...
**预加载：**顾名思义就是预先加载，再进一步解释就是还没有使用该单例对象，但是单例对象就已经呗加载到内存了
...
**懒加载**：为避免内存的浪费，我们可以采用懒加载，即用到该单例对象的时候再创建
...
**概念**：简单工厂包括抽象产品，具体产品，具体工厂 ，将实例化对象提取出来，放到一个类中统一管理和维护，达到和主项目的依赖关系解耦
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/5c4b6c81-a320-4daa-a3c3-637cf3e11f2b.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/a8ed0645-b7d9-4fea-8363-fb8d4a391f34.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/d595cbb3-4686-451b-bcac-ca572f23ea60.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/3058a9a0-0f2c-4c1e-b10d-a9dd7706478c.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/8ec36526-9e0a-43dd-ac33-715e812a8cf1.png)

**举例**（我们如果构建生成一台电脑，那么我们可能需要这么几个步骤（1）需要一个主机（2）需要一个显示器（3）需要一个键盘（4）需要一个鼠标）

虽然我们具体在构建一台主机的时候，每个对象的实际步骤是不一样的，比如，有的对象构建了i7cpu的主机，有的对象构建了i5cpu的主机，有的对象构建了普通键盘，有的对象构建了机械键盘等。但不管怎样，你总是需要经过一个步骤就是构建一台主机，一台键盘。对于这个例子，我们就可以使用生成器模式来生成一台电脑，他需要通

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/e1a11ed0-7b4a-4461-9d77-f803df2e1029.png)
...
举例（将 USB 接口转换为 VGA 接口）类图如下

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/594df954-e19b-4801-8f2a-d8c673aef9c7.png)
...
对象适配器和类适配器使用了不同的方法实现适配，对象适配器组合，类适配器使用继承
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/1467410d-9cae-47b1-858b-9d22fe738858.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/b03859dc-4c7c-473d-9033-42934af38ac9.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/ba7609ee-f459-4881-beef-7639ae783973.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/de0ef138-a0e2-4c6f-bdf3-8ef3c30744bc.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/f192f9f2-078b-4753-983e-acc17150f57b.png)
...
**应用：**电商系统下单流程，家庭影院系统，计算机启动过程

**模式结构和代码示例**

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/35230f06-b4c5-46e7-a8ef-f55d4631ba25.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/610184d9-06e5-4ec7-a64a-9085e6c5ad9d.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/c9e351af-f27b-41d4-864b-5fbabdedfe4d.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/a74bb81e-eb14-4966-9a24-ad58bb3e9bde.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/78de29c8-6636-4e27-82d6-022d29698498.png)

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/55b8b59c-7ea1-4bf6-8750-dc545c7e9df1.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/972c9b6d-b61d-4f4a-b129-36dd1fd50a82.png)
...
- 树枝构件（Composite）角色：是组合中的分支节点对象，它有子节点。它实现了抽象构件角色中声明的接口，它的主要作用是存储和管理子部件，通常包含 Add()、Remove()、GetChild() 等方法
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/a5256d96-2ff0-48d6-b7a9-05b23909f722.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/ee90f748-153a-46a5-8090-aba4794a7278.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/d8225f3d-074f-4d4a-8451-74c97c376241.png)
...
3. 创建工厂，这里要特别注意，为了避免享元对象被重复创建，使用 Hashmap 确保 key 保持一致
...
**应用场景：**多种支付方式处理，折扣策略应用，角色行为策略

**策略模式结构和示例代码**

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/22534117-331c-49bf-9380-ef987c099da5.png)
...
举例如下（ 实现一个加减的功能），类图如下：
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/6d141033-3291-4362-9db3-fb1f01898f07.png)
...
**应用场景：**Web 框架中的请求处理流程，数据处理流程，编译器的编译流程，订单处理流程

**模型结构和代码示例：**

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/268ffaf5-0e0c-4111-a67b-01ecd2af5530.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/b4c5476f-9c87-4a5b-8977-d78507749055.png)
...
**应用场景**：GUI 事件处理，消息队列系统，系统监控和报警

**模式结构图和代码示例**

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/8106f800-8096-4785-8490-4ec08cd82553.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/65b252fa-23ad-4d22-9f6d-db0c50139aa0.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/7ea6c546-ab0f-4310-a5ad-25d25cb4e84e.png)
...
举例（咖啡厅和中餐厅合并，他们两个餐厅的菜单一个是数组保存的，一个是 ArrayList 保存的，遍历方式不一样，使用迭代器聚合访问只需要一种方式）

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/7ddc2d38-6a3d-4c79-8a05-593f58ce328d.png)
...
**应用场景：**审批流程，异常处理系统，过滤网链，Web 框架的中间件处理，不同级别日志处理

**模式结构和代码示例**：

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/64fdb487-3f9c-4004-a431-f8ab37f07df9.png)
...
举例（购买请求决策，价格不同要由你不同的级别决定，组长， 部长，副部，总裁）类图如下：

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/b4fe9c35-8ca0-4990-84ff-fcedd59afecf.png)

1 决策抽象类，包含对请求处理的函数，同时还包含指定下一个决策者的函数
...
**应用场景**：图形界面操作，任务队列与日志，遥控器或智能家具控制

**模式结构和代码示例**

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/306f5eb6-fe31-4175-bd0a-0b81b34c9866.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/9058b45f-7648-49f1-9cbd-7dc308eef2a8.png)
...
**应用场景：**游戏角色行为切换，订单状态流转，审批流程，工单状态管理

**模式结构和代码示例**

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/cf473189-df08-47e5-8ab8-64e9eae5047e.png)

- **State 抽象状态角色**

接口或抽象类，负责对象状态定义，并封装环境角色以实现状态切换
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/f0f57a9e-c29c-4ac1-bdae-53286347e665.png)
...
**应用场景：** 撤销/重做功能， 游戏存档读档，事务回滚，浏览历史记录

**结构图和代码示例**

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/a6da704e-ea27-4af7-b733-d10305788342.png)
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/1e8e4f40-8923-4481-b3c5-b6fdda473c60.png)
...
**定义**：访问者模式的核心思想是**将数据结构和操作分离**，允许在不修改数据结构的前提下，定义新的操作。
...
**应用场景：** 编译器/解释器，文档转换，UI 事件处理，数据导出

**模式结构和代码示例**

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/713c1d0d-8069-4551-b7b5-789cf5a88bcf.png)
...
class ComputerPartDisplayVisitor implements ComputerPartVisitor {
    ...
    public void visit(Keyboard keyboard) {
        System.out.println("显示键盘信息: " + keyboard.getInfo());
    }

    @Override
    public void visit(Monitor monitor) {
        System.out.println("显示显示器信息: " + monitor.getInfo());
    }

    @Override
    public void visit(Mouse mouse) {
        System.out.println("显示鼠标信息: " + mouse.getInfo());
    }

    @Override
    public void visit(Computer computer) {
        System.out.println("=== 开始显示电脑配置 ===");
        for (ComputerPart part : computer.getParts()) {
            part.accept(this);
        }
        System.out.println("=== 电脑配置显示完成 ===\n");
    }
}
```
...
其主要**缺点**是：当同事类太多时，中介者的职责将很大，它会变得复杂而庞大，以至于系统难以维护。

**模式结构和代码示例**

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/4f63e34d-e7e7-4ed6-b8b0-ec303a7a9aa0.png)
...
**应用场景：**聊天室，事件总线，UI组件管理

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/58f23453-032f-4704-b0a7-65cda2177fc8.png)
